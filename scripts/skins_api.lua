...
AddClassPostConstruct("screens/redux/lobbyscreen", function(self, profile, cb)
    local function OnUserPickedCharacter(char, skin_base, clothing_body, clothing_hand, clothing_legs, clothing_feet)
        local function doSpawn()
            ...
            _G.TheNet:SendSpawnRequestToServer(char, skin_base, clothing_body, clothing_hand, clothing_legs,
                clothing_feet)
...
AddClassPostConstruct("screens/redux/wardrobepopupgridloadout", function(self)
    ...
    function self:Close(...)
        if self.owner_player and SKINNABLE_CHARACTERS[self.owner_player.prefab] then
            ...
            if not data.base or data.base == self.loadout.currentcharacter or data.base == "" then
                data.base = (self.loadout.currentcharacter .. "_none")
            end
            if not IsValidClothing(data.body) or not TheInventory:CheckOwnership(data["body"]) then
                data.body = ""
            end
            if not IsValidClothing(data.hand) or not TheInventory:CheckOwnership(data["hand"]) then
                data.hand = ""
            end
            if not IsValidClothing(data.legs) or not TheInventory:CheckOwnership(data["legs"]) then
                data.legs = ""
            end
            if not IsValidClothing(data.feet) or not TheInventory:CheckOwnership(data["feet"]) then
                data.feet = ""
            end
...
AddClassPostConstruct("screens/redux/skinpresetspopup",
    function(self) -- For the base option to show up again for mod characters in the skin presets
        ...
        if SKINNABLE_CHARACTERS[self.character] then
            ...
            local function ScrollWidgetsCtor(context, i)
                ...
                item.load_btn =
                    item.root:AddChild(TEMPLATES.IconButton("images/button_icons.xml", "apply_skins.tex", nil, nil, nil,
                        function(a)
                            self:_LoadPreset(item.i)
                        end, _G.STRINGS.UI.SKIN_PRESETS.LOAD))
                item.load_btn:SetPosition(105, -1)
                item.load_btn:SetScale(0.7)

                item.save_btn = item.root:AddChild(TEMPLATES.IconButton("images/button_icons.xml", "save.tex", nil, nil,
                    nil, function()
                        self:_SetPreset(item.i)
                    end, _G.STRINGS.UI.SKIN_PRESETS.SAVE))
            ...
            self.scroll_list = self.proot:AddChild(TEMPLATES.ScrollingGrid(self.list_items, {
                context = {},
...
function _G.SpawnNewPlayerOnServerFromSim(player_guid, skin_base, clothing_body, clothing_hand, clothing_legs,
    clothing_feet, starting_item_skins, skillselection)
    ...
    if player ~= nil and SKINNABLE_CHARACTERS[player.prefab] then
        ...
        _SpawnNewPlayerOnServerFromSim(player_guid, skin_base, clothing_body, clothing_hand, clothing_legs,
            clothing_feet, starting_item_skins, skillselection)
    end
end
