...
AddClassPostConstruct("screens/redux/lobbyscreen", function(self, profile, cb)
	local function OnUserPickedCharacter(char, skin_base, clothing_body, clothing_hand, clothing_legs, clothing_feet)
		local function doSpawn()
			...
			_G.TheNet:SendSpawnRequestToServer(
				char,
				skin_base,
				clothing_body,
				clothing_hand,
...
AddClassPostConstruct(
	"screens/redux/skinpresetspopup",
	function(self) --For the base option to show up again for mod characters in the skin presets
		...
		if SKINNABLE_CHARACTERS[self.character] then
			...
			local function ScrollWidgetsCtor(context, i)
				...
				item.load_btn = item.root:AddChild(
					TEMPLATES.IconButton("images/button_icons.xml", "apply_skins.tex", nil, nil, nil, function(a)
				...
				item.save_btn = item.root:AddChild(
					TEMPLATES.IconButton("images/button_icons.xml", "save.tex", nil, nil, nil, function()
...
function _G.SpawnNewPlayerOnServerFromSim(
	player_guid,
	skin_base,
	clothing_body,
	clothing_hand,
	...
	if player ~= nil and SKINNABLE_CHARACTERS[player.prefab] then
		...
		_SpawnNewPlayerOnServerFromSim(
			player_guid,
			skin_base,
			clothing_body,
			clothing_hand,
