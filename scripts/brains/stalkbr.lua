...
local stalkbr = Class(Brain, function(self, inst)
...
local function IsDefensive(self)
    return self.inst.components.health.currenthealth > 999999999999
end
...
local function ShouldSummonChannelers(self)
    return IsDefensive(self) and self.inst.components.commander:GetNumSoldiers() <= 0 and
               not self.inst.components.timer:TimerExists("channelers_cd")
end

local function ShouldSummonMinions(self)
    return not self.hasminions and IsDefensive(self) and not self.inst.components.timer:TimerExists("minions_cd")
end
...
local function ShouldUseAbility(self)
    ...
    self.abilityname = self.inst.components.combat:HasTarget() and
                           ((ShouldSnare(self) and "fossilsnare") or (ShouldSpikes(self) and "fossilspikes")) or nil
...
function stalkbr:OnStart()
    local root

    if self.inst.atriumstalker then
        root = PriorityNode({WhileNode(function()
            return ShouldUseAbility(self)
        end, "Ability", ActionNode(function()
            self.inst:PushEvent(self.abilityname, self.abilitydata)
            self.abilityname = nil
            self.abilitydata = nil
        end)), WhileNode(function()
            return ShouldFeast(self)
        end, "FossilFeast", ActionNode(function()
            self.inst:PushEvent("fossilfeast")
        end)), ChaseAndAttackAndAvoid(self.inst, GetStargate, AVOID_GATE_DIST),

                             Follow(self.inst, GetLeader, MIN_FOLLOW_DIST, TARGET_FOLLOW_DIST, MAX_FOLLOW_DIST),
                             Wander(self.inst), WhileNode(function()
    ...
    elseif self.inst.canfight then
        root = PriorityNode({WhileNode(function()
            return self.inst.components.combat:HasTarget() and ShouldSnare(self)
        end, "FossilSnare", ActionNode(function()
            self.inst:PushEvent("fossilsnare", self.abilitydata)
            self.abilitydata = nil
        end)), ChaseAndAttack(self.inst),
                             ParallelNode({SequenceNode({WaitNode(RESET_COMBAT_DELAY), ActionNode(function()
            self.inst:SetEngaged(false)
        end)}), PriorityNode({SequenceNode({FindClosest(self.inst, SEE_LURE_DIST, SAFE_LURE_DIST, {"shadowlure"}),
                                            FaceEntity(self.inst, GetShadowLure, KeepShadowLure)}), Wander(self.inst)},
            0.5)})}, 0.5)
    else
        local t = GetTime()
        ...
        root = PriorityNode({WhileNode(function()
        ...
        end, "Daytime", ActionNode(function()
            self.inst:PushEvent("flinch")
        end)), WhileNode(function()
            local t = GetTime()
            ...
            return false
        end, "SkullAche", ActionNode(function()
            self.inst:PushEvent("skullache")
        end)), WhileNode(function()
            local t = GetTime()
            ...
            return false
        end, "FallApart", ActionNode(function()
            self.inst:PushEvent("fallapart")
        end)), SequenceNode({FindClosest(self.inst, SEE_LURE_DIST, SAFE_LURE_DIST, {"shadowlure"}),
                             FaceEntity(self.inst, GetShadowLure, KeepShadowLure)})}, 0.5)
    end
