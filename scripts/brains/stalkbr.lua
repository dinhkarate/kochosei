...
local function ShouldCombatFeast(self)
	...
	if
		not self.inst.hasshield
		and self.inst.components.combat:GetLastAttackedTime() + COMBAT_FEAST_DELAY >= GetTime()
	then
		return false
	end
...
local function ShouldUseAbility(self)
	...
	self.abilityname = self.inst.components.combat:HasTarget()
			and ((ShouldSnare(self) and "fossilsnare") or (ShouldSpikes(self) and "fossilspikes")
)
		or nil
