local function onfreeze(inst, target)
	...
	if target.components.freezable ~= nil then
		target.components.freezable:AddColdness(
			10,
			10 + Remap((inst.crab and inst.crab:IsValid() and inst.crab.gemcount.blue or 0), 0, 9, 0, 10)
		)
...
local function dofreezefz(inst)
	...
	local time = 0.1
	inst.freezetask = inst:DoTaskInTime(time, function()
		inst.freezefx(inst)
	end)
end
...
local function freezefx(inst)
	local function spawnfx()
		local MAXRADIUS = inst.crab and inst.crab:IsValid() and inst.crab:GetFreezeRange()
			or (TUNING.CRABKING_FREEZE_RANGE * 0.75)
	...
	local fx = Remap(
		inst.components.age:GetAge(),
		0,
		TUNING.CRABKING_CAST_TIME_FREEZE
			- (math.min((inst.crab and inst.crab:IsValid() and math.floor(inst.crab.gemcount.yellow / 2) or 0), 4)),
		1,
		MAXFX
	)
...
local function dofreeze(inst)
	...
	local range = inst.crab and inst.crab:IsValid() and inst.crab:GetFreezeRange()
		or (TUNING.CRABKING_FREEZE_RANGE * 0.75)
	local ents = TheSim:FindEntities(pos.x, pos.y, pos.z, range, nil, FREEZE_CANT_TAGS)
	for i, v in pairs(ents) do
		if v.components.temperature then
			local rate = (
				TUNING.CRABKING_BASE_FREEZE_AMOUNT
				+ (
					(inst.crab and inst.crab:IsValid() and inst.crab.gemcount.blue or 0)
					* TUNING.CRABKING_FREEZE_INCRAMENT
				)
			)
				/ (
					(
						TUNING.CRABKING_CAST_TIME_FREEZE
						- (inst.crab and inst.crab:IsValid() and math.floor(inst.crab.gemcount.yellow / 2) or 0)
					) / interval
				)
	...
	local time = 0.2
	inst.lowertemptask = inst:DoTaskInTime(time, function()
		inst.dofreeze(inst)
	end)
end
