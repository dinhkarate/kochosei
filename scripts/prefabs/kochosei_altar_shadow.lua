...
local function OnLevelUp(inst, data)
	if
		...
		and data ~= nil
		and data.source ~= nil
		and data.source.prefab ~= inst.prefab
		...
		and data.source.level > #inst.levelupsource
		and not table.contains(inst.levelupsource, data.source.prefab)
	then
...
local function knightlevelup(inst, overridelevel)
	...
	if level ~= nil then
		...
		inst.components.combat:SetRange(
			TUNING.SHADOW_KNIGHT.ATTACK_RANGE * scale,
...
local function bishoplevelup(inst, overridelevel)
	...
	if level ~= nil then
		...
		inst.components.combat:SetRange(
			TUNING.SHADOW_BISHOP.ATTACK_RANGE[level],
			TUNING.SHADOW_BISHOP.HIT_RANGE * scale
		)
...
local function bishopfn()
	local inst = commonfn("kochosei_altar_shadow", true)
...
return Prefab(
	"kochosei_altar_shadow",
	bishopfn,
	createassets("kochosei_altar_shadow"),
	createprefabs({
		"kochosei_altar_shadow_fx" --[[, "shadow_mitre", "shadow_sceptre"]],
	})
),
