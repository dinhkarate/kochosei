local function make(code, description, check_des_boolean, alter_description, fileanim)
    ...
    STRINGS.CHARACTERS.GENERIC.DESCRIBE[string.upper(code)] =
        alter_description or STRINGS.SKIN_DESCRIPTIONS[code] or
    ...
    return Prefab(code, function()
        ...
        inst.components.equippable:SetOnUnequip(function(inst, owner)
            if not (owner and owner.components.health and owner:HasTag("player")) then
                return
            end
            ...
            if code == "kochosei_card_defend" then
                health:SetMaxDamageTakenPerHit(nil)
            end

            if code == "kochosei_card_attack" then
                if owner.components.combat and owner.components.combat.damagemultiplier then
                    owner.components.combat.damagemultiplier = owner.components.combat.damagemultiplier / 2
                end
            end

            if code == "kochosei_card_health" then
                -- chỉ trừ khi còn đủ máu gốc
                if old_max > 500 then
                    local new_max = old_max - 500

                    -- set max trước
                    health:SetMaxHealth(new_max)

                    -- giữ % máu
                    local new_current = math.floor(new_max * health_percent + 0.5)
                    ...
                    health:SetCurrentHealth(new_current)
                end
            end
