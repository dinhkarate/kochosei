...
local function onattack(inst, attacker, target)
    if target ~= nil and target:IsValid() and target.components.burnable ~= nil and math.random() < TUNING.LIGHTER_ATTACK_IGNITE_PERCENT
        * target.components.burnable.flammability then target.components.burnable:Ignite(nil, attacker) end
end
...
local function SetTemperatureToOwner(owner)
    if owner and owner.components.temperature then
       ...
       if current and current <= 3 then owner.components.temperature:SetTemperature(3) end
    end
end
...
local function checklight(inst)
    ...
    for i, v in ipairs(playersheal) do 
        ...
        if owner2 == nil then
            SetTemperatureToOwner(v) end
        end

    if inst.components.equippable:IsEquipped() then
        local owner = inst.components.inventoryitem.owner
        if owner and owner:HasTag("player") then SetTemperatureToOwner(owner) end
    end

    if inst:HasTag("INLIMBO") then
        if inst.firestest ~= nil then
            for i, fx in ipairs(inst.firestest) do fx:Remove() end
            inst.firestest = nil
        end
    else
        if inst.firestest == nil then
