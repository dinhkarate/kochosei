...
local book_defs =
{
		...
		local function tryharvest(inst) local objc = inst.components if objc.crop ~= nil then objc.crop:Harvest(reader) elseif objc.harvestable ~= nil then objc.harvestable:Harvest(reader) elseif objc.stewer ~= nil then objc.stewer:Harvest(reader) elseif objc.dryer ~= nil then objc.dryer:Harvest(reader) elseif objc.occupiable ~= nil and objc.occupiable:IsOccupied() then local item = objc.occupiable:Harvest(reader) if item ~= nil then reader.components.inventory:GiveItem(item) end elseif objc.pickable ~= nil and objc.pickable:CanBePicked() then objc.pickable:Pick(reader) end end
