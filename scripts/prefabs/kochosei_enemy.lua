...
local function NhanItemCuaDinhCute(inst, item)
	...
	return items[dinh]
		or item.components.equippable
			...
			and not item.components.projectile -- check if the item is not a projectile
end
...
local function HandleEquippable(inst, item)
	...
	if
		...
		and (
			...
			or equippable.equipslot == EQUIPSLOTS.BODY
		)
	then
...
local function retargetfn(inst)
	...
	return FindEntity(inst, TUNING.SHADOWWAXWELL_TARGET_DIST, function(guy)
		if leader ~= nil then
			return guy ~= inst and guy.components.combat and guy.components.health and (guy.components.combat:TargetIs(
				leader
			) or guy.components.combat:TargetIs(inst)) and inst.components.combat:CanTarget(guy),
				{ "_combat" },
				{ "playerghost", "INLIMBO" }
		else
			return (
