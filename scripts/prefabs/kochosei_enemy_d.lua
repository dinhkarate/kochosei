...
local RETARGET_CANT_TAGS = {
    "DECOR", "eyeofterror", "FX", "INLIMBO", "NOCLICK", "notarget",
    "playerghost", "wall"
}
local function RetargetFn(inst)
    return FindEntity(inst,
                      inst.range or TUNING.DECID_MONSTER_TARGET_DIST * 3.5,
                      function(guy)
...
local function KeepTargetFn(inst, target)
    return target ~= nil and not target.components.health:IsDead() and
               inst.components.combat:CanTarget(target) and
               inst:IsNear(target, 20)
end
local function IsList(item) return item.prefab == "ruinshat" end
local function IsList2(item) return item.prefab == "blowdart_sleep" end
local function IsList3(item) return item.prefab == "armorruins" end
local function IsList4(item) return item.prefab == "ruins_bat" end
...
local function DoRipple(inst)
    if inst.components.drownable ~= nil and
        inst.components.drownable:IsOverWater() then
        SpawnPrefab("weregoose_ripple" .. tostring(math.random(2))).entity:SetParent(
            inst.entity)
    end
end
...
local function common()
    ...
    inst:DoPeriodicTask(.2, DoRipple, FRAMES)
...
return Prefab("kochosei_enemy_d", fnd, assets, prefabs),
       Prefab("kochosei_enemy_c", fnc, assets, prefabs)
