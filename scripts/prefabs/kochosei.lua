...
local function GetKochoMap(inst)
	local x, y, z = inst.Transform:GetWorldPosition()
	local stone = TheSim:FindFirstEntityWithTag("cay_kocho")
	if stone ~= nil then
...
if TUNING.KOCHOSEI_CHECKMOD ~= 1 then
    ...
    Kochoseiapi.MakeCharacterSkin("kochosei", "kochosei_snowmiku_skin1", {
        ...
        des = "o((>ω< ))o",
        quotes = "Sao không mang ra test luôn, all in nó bị phèn thì ai sửa?,t à hay dinh hay ông?, mà chắc là ông rồi :)",
        rarity = "Elegant",
        skins = {
