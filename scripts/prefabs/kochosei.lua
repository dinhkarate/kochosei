...
local function onemote(inst, data)
	local soundname = data.soundoverride
		or (type(data.anim) == "table" and (type(data.anim[1]) == "table" and data.anim[1][1] or data.anim[1]))
		or (type(data.anim) == "string" and data.anim)
		or "emote"
...
local function OnHitOther(inst, data)
	...
	if
		item ~= nil
		and item:HasTag("kochoseiweapon")
		and target ~= nil
		and target.components.health
		and target.components.combat
	then
		if target:HasTag("epic") then
			inst.components.sttptmau:SatthuongWP(
				target,
				TUNING.MIOHM_SAT_THUONG_PT_MAU * 2,
				false,
				false,
...
local function lai_nhai(inst)
	...
	if inst.components.talker then
		inst.components.talker:Say(
			"Điểm waifu hiện có: "
				.. TUNING.KOCHOSEI_CHECKWIFI
				.. "\n Búa max damage: "
				.. TUNING.KOCHOSEI_MAX_LEVEL + (TUNING.KOCHOSEI_CHECKWIFI * 2)
				.. "\n Nơ kháng "
				.. TUNING.KOCHO_HAT1_ABSORPTION * 100
				.. "% damage"
				.. " có "
				.. laydoben
				.. " điểm độ bền",
