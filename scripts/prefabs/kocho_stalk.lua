...
--	local v01 = TheSim:FindEntities(x, y, z, TUNING.STALKER_DEAGGRO_DIST, nil, {"character", "structure"}, {"hostile", "epic", "berrythief", "mossling"})
   ...
   if v01 then  for i, v in ipairs(v01) do
...
--        hasplayer = target:HasTag("player")
        hasplayer = target:HasTag("hostile") or target:HasTag("epic") or target:HasTag("berrythief") or target:HasTag("mossling")
        if hasplayer then
...
--    if target ~= nil and target:HasTag("player") then
    if target ~= nil and (target:HasTag("hostile") or target:HasTag("epic") or target:HasTag("berrythief") or target:HasTag("mossling"))  then
        local newplayer = inst.components.grouptargeter:TryGetNewTarget()
...
local function FindSnareTargets(inst)
    ...
    for i, v in ipairs(ents) do
        if not (v.components.health ~= nil and v.components.health:IsDead()) and
            (not inatrium or inst:IsNearAtrium(v)) then
            if (v:HasTag("hostile") or v:HasTag("epic") or v:HasTag("berrythief")or v:HasTag("monster") or v:HasTag("mossling")) then
