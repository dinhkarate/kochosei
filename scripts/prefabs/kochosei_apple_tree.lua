...
SetSharedLootTable("kochosei_apple_tree_normal", -- kochosei_applestree_normal
{{"log", 1.0}, {"log", 1.0}, {"log", 1.0}, {"petals", 1.0}, {"petals", 1.0}})

SetSharedLootTable("kochosei_apple_tree_tall", -- kochosei_applestree_tall
{
    {"log", 1.0}, {"log", 1.0}, {"log", 1.0}, {"log", 1.0}, {"log", 1.0},
    {"kochosei_apple", 1.0}, {"kochosei_apple", 1.0}, {"kochosei_apple", 1.0},
    {"kochosei_apple", 1.0}, {"kochosei_apple", 1.0}
})

SetSharedLootTable("kochosei_apple_tree_burnt", -- kochosei_applestree_burnt
    ...
    if not (chopper ~= nil and chopper:HasTag("playerghost")) then
        inst.SoundEmitter:PlaySound(
            chopper ~= nil and chopper:HasTag("beaver") and
                "dontstarve/characters/woodie/beaver_chop_tree" or -- con woodie biến hình
...
local function MakeStump(inst)
    ...
    if inst.components.timer ~= nil and
        not inst.components.timer:TimerExists("decay") then
...
local function MakeBurnt(inst, immediate)
    ...
    if inst.components.growable ~= nil then inst:RemoveComponent("growable") end
...
local function GetStatus(inst)
    return (inst:HasTag("burnt") and "BURNT") or
               (inst:HasTag("stump") and "CHOPPED") or
               (inst.components.burnable ~= nil and
                   inst.components.burnable:IsBurning() and "BURNING") or nil
end
------------------------------------------------------------------------------------------------NOTE TO Ở kochosei_apple_tree_anims
local function Sway(inst)
    local anim_to_play = (math.random() > 0.5 and
                             kochosei_apple_tree_anims[inst.size].sway1) or
                             kochosei_apple_tree_anims[inst.size].sway2
...
local function kochosei_apple_tree(name, stage, data)
    local function fn()
        ...
        inst.size = (stage == 1 and SHORT) or (stage == 2 and NORMAL) or
                        (stage == 3 and TALL) or nil
        ...
        --		inst.components.lootdropper:SetChanceLootTable('cherry1') --đây hình như là hạt giống của nó

        inst:AddComponent("workable")
        inst.components.workable:SetWorkAction(ACTIONS.CHOP)
        inst.components.workable:SetWorkLeft(
            TUNING.KOCHOSEI_APPLE_TREE_CHOPS_TALL)
        ...
        inst.components.growable:SetStage(
            stage == 0 and math.random(1, 3) or stage)
        ...
        if data == "stump" then
            ...
            inst.AnimState:PlayAnimation(
                kochosei_apple_tree_anims[inst.size].stump)
        elseif data == "burnt" then
            MakeBurnt(inst, true)
        end

        if POPULATING then Sway(inst) end
...
STRINGS.CHARACTERS.GENERIC.DESCRIBE.KOCHOSEI_APPLE_TREE =
    "A lovely tree in my memories!"
