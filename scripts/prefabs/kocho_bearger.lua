local assets = {
    ...
    Asset("ANIM", "anim/bearger_yule.zip"), Asset("SOUND", "sound/bearger.fsb")
}

local prefabs = {
    "groundpound_fx", "groundpoundring_fx", "bearger_fur", "furtuft", "meat",
    "chesspiece_bearger_sketch", "collapse_small"
}

local brain = require("brains/kochobeargerbrain")
local RETARGET_MUST_TAGS = {"monster"}
local RETARGET_CANT_TAGS = {
    "prey", "smallcreature", "INLIMBO", "player", "structure","kochoseipet"
}
...
local function RetargetFn(inst)
    ...
    return FindEntity(inst, TARGET_DIST, function(guy)
        return inst.components.combat:CanTarget(guy) and
                   (guy.components.combat:TargetIs(inst) or
...
local function WorkEntities(inst)
    ...
    for i, v in ipairs(TheSim:FindEntities(x, 0, z, 5, nil, WORKABLES_CANT_TAGS,
                                           WORKABLES_ONEOF_TAGS)) do
...
local function fn()
    ...
    inst.AnimState:SetBuild(
        IsSpecialEventActive(SPECIAL_EVENTS.WINTERS_FEAST) and "bearger_yule" or
            "bearger_build")
    ...
    inst.components.combat:SetHurtSound(
        "dontstarve_DLC001/creatures/bearger/hurt")
    ...
    inst.components.groundpounder.noTags = {
        "FX", "NOCLICK", "DECOR", "INLIMBO", "structure"
    }
