            ...
            if v.components.timer and v:HasTag("player") then
                if v.components.timer:TimerExists("kochobuffspeed_time") then
                    ...
                    if v.components.talker then
                        v.components.talker:Say("Speed Buff From Kochosei", 5,
                                                false)
                    end
                else
                    v.components.timer:StartTimer("kochobuffspeed_time", 20)
                    v.components.locomotor:SetExternalSpeedMultiplier(v,
                                                                      "kochosei_speed_mod",
                                                                      2)
                    fx.Transform:SetPosition(v.Transform:GetWorldPosition())
                    if v.components.talker then
...
local function OnDischarged(inst)
    if inst.components.book ~= nil then inst:RemoveComponent("book") end
...
STRINGS.CHARACTERS.GENERIC.DESCRIBE.KOCHOSEI_ANCIENT_BOOKS =
    "Woah! Its secrets of Shinobi"
STRINGS.RECIPE_DESC.KOCHOSEI_ANCIENT_BOOKS =
    "Secrets of Shinobi, increase your speed in 20s and cooldown 3 min"
