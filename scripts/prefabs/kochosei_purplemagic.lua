local assets = {Asset("ANIM", "anim/kochosei_purplemagic.zip"), Asset("ANIM", "anim/swap_kochosei_purplemagic.zip")}
...
local function CastSpawnClone(inst, target, pos)
    ...
    if check <= TUNING.KOCHOSEI_SLAVE_COST then
        caster.components.talker:Say("Không đủ Sanity, clone bóng tối sẽ được triệu hồi")
        local cloneshadow = caster.components.petleash:SpawnPetAt(pos.x, 0, pos.z, "kochosei_enemy")
        if cloneshadow ~= nil then
            cloneshadow.sg:GoToState("spawn")
            cloneshadow.AnimState:SetMultColour(0, 0, 0, 0.6)

            cloneshadow:AddTag("shadowminion")
            cloneshadow:AddTag("shadow_aligned")
            cloneshadow.components.health:SetMaxHealth(TUNING.KOCHOSEI_SLAVE_HP * 2)
            cloneshadow.components.combat:SetDefaultDamage(TUNING.KOCHOSEI_SLAVE_DAMAGE * 2)
            cloneshadow:DoPeriodicTask(3, function()
                if caster.components.sanity and caster.components.sanity.current >= TUNING.KOCHOSEI_SLAVE_COST then
                    cloneshadow.components.health:Kill()
                end
            end)
        end
    else
...
local function fn()
    ...
    inst.components.spellcaster:SetSpellFn(CastSpawnClone)
