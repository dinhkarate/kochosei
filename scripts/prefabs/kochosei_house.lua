...
local kochosei_house_assets = {Asset("ANIM", "anim/kochosei_house.zip"),
...
local function startsleepsound(inst, len)
    stopsleepsound(inst)
    inst.sleep_tasks = {inst:DoPeriodicTask(len, PlaySleepLoopSoundTask, 33 * FRAMES),
...
local function onwake(inst, sleeper, nostatechange)
    ...
    if sleeper and sleeper:HasTag("kochosei") then
        ...
        for _, v in pairs(pet) do
            if v.prefab == "kochosei_enemy" or v.prefab == "kochosei_enemyb" then
