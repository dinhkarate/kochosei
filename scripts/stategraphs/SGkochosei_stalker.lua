...
local function ShakeRoar(inst)
	ShakeAllCameras(CAMERASHAKE.FULL, 1.2, 0.03, 0.7, inst, 30)
end

local function ShakeSummonRoar(inst)
	ShakeAllCameras(CAMERASHAKE.FULL, 0.7, 0.03, 0.4, inst, 30)
end

local function ShakeSummon(inst)
	ShakeAllCameras(CAMERASHAKE.VERTICAL, 0.5, 0.02, 0.2, inst, 30)
end

local function ShakePound(inst)
	ShakeAllCameras(CAMERASHAKE.VERTICAL, 0.5, 0.03, 0.7, inst, 30)
end

local function ShakeMindControl(inst)
	ShakeAllCameras(CAMERASHAKE.FULL, 2, 0.04, 0.075, inst, 30)
end

local function ShakeDeath(inst)
	ShakeAllCameras(CAMERASHAKE.VERTICAL, 0.6, 0.02, 0.4, inst, 30)
end
...
local function BlinkHigh(inst)
	SetBlinkLevel(inst, 1)
end
local function BlinkMed(inst)
	SetBlinkLevel(inst, 0.3)
end
local function BlinkLow(inst)
	SetBlinkLevel(inst, 0.2)
end
local function BlinkOff(inst)
	SetBlinkLevel(inst, 0)
end
...
local function PickShield(inst)
	local t = GetTime()
	if (inst.sg.mem.lastshieldtime or 0) + 0.2 >= t then
		return
	end
	...
	return rnd < dt / (MAIN_SHIELD_CD * 2) + 0.5 and 2 or 1
end
...
local events = {
	...
	EventHandler("fossilsnare", function(inst, data)
		if
			not (inst.sg:HasStateTag("busy") or inst.components.health:IsDead())
			and data ~= nil
			and data.targets ~= nil
			and #data.targets > 0
		then
		...
		if not inst.components.health:IsDead() then
			if inst.hasshield then
				...
				if shieldtype ~= nil then
					...
					if shieldtype < 3 and math.random() < 0.5 then
			...
			if
				...
				and not CommonHandlers.HitRecoveryDelay(inst, TUNING.STALKER_HIT_RECOVERY)
			then
	...
	EventHandler("flinch", function(inst)
		...
		if
			not (inst.sg:HasStateTag("flinching") or inst.components.health:IsDead())
			and (not inst.sg:HasStateTag("busy") or inst.sg:HasStateTag("caninterrupt"))
		then
...
local states = {
	State({
		...
		timeline = {
			TimeEvent(0 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/in")
			end),
	...
	State({
		...
		onenter = function(inst)
			...
			inst.sg.statemem.baselightoverride = 0.1
			if inst.foreststalker then
				inst:StopBlooming()
			end
		end,
		...
		timeline = {
			TimeEvent(0 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/enter")
			end),
		...
		onupdate = function(inst)
			if inst.sg.statemem.fadeout ~= nil then
				if inst.sg.statemem.fadeout > 0.02 then
					inst.sg.statemem.fadeout = inst.sg.statemem.fadeout - 0.02
					inst.AnimState:SetLightOverride(inst.sg.statemem.fadeout)
				else
					inst.sg.statemem.fadeout = nil
					inst.AnimState:SetLightOverride(0)
				end
			elseif inst.sg.statemem.baselightoverride < 0.2 then
				inst.sg.statemem.baselightoverride = math.min(0.2, inst.sg.statemem.baselightoverride + 0.01)
	...
	State({
		...
		timeline = {
			TimeEvent(0 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/footstep")
			end),
	...
	State({
		...
		timeline = {
			TimeEvent(5 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/death_walk")
			end),
	...
	State({
		...
		timeline = {
			TimeEvent(0, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/death3/swell")
			end),
			TimeEvent(FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/death_pop")
			end),
			TimeEvent(6 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/death3/whip")
			end),
			TimeEvent(15 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/death3/whip_snap")
			end),
			TimeEvent(41 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/death3/pianohits_1")
			end),
			TimeEvent(44 * FRAMES, ShakeIfClose),
			TimeEvent(49 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/death3/orchhits")
			end),
			TimeEvent(55 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/death3/stretch")
			end),
			...
			TimeEvent(108 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/death3/pianohits_1")
			end),
			...
			TimeEvent(116 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/death3/orchhits")
			end),
			TimeEvent(132 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/death3/whip_snap")
			end),
			...
			TimeEvent(152 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/death3/pianohits_2")
			end),
			TimeEvent(155 * FRAMES, ShakeDeath),
			TimeEvent(168 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/death3/stretch")
			end),
			...
			TimeEvent(190 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/death3/transform")
			end),
			...
			TimeEvent(300 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/charlie/attack_low")
			end),
	...
	State({
		...
		timeline = {
			...
			TimeEvent(13 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/attack_swipe")
			end),
			TimeEvent(32 * FRAMES, function(inst)
	...
	State({
		...
		timeline = {
			...
			TimeEvent(48 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/taunt_short", nil, 0.6)
			end),
	...
	State({
		...
		onenter = function(inst, data)
			...
			if ishurt and #inst:FindMinions() > 0 then
				...
				return
			elseif
				data.idle ~= nil and (not ishurt or data.idle > 6 or data.resist <= 0 or #inst:FindMinions(5) <= 0)
			then
	...
	State({
		...
		timeline = {
			TimeEvent(10 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/hurt")
			end),
			TimeEvent(12 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/out")
			end),
	...
	State({
		...
		timeline = {
			...
			TimeEvent(50 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/arm")
			end),
		},
