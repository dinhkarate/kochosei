...
local function DoSpawnIceSpike(inst, x, z)
	...
	if #ents > 0 then
		for i, ent in ipairs(ents) do
			if ent ~= inst then
				if
					not inst._icespikeshit_targets[ent.GUID]
					and inst.components.combat:CanTarget(ent)
					and not ent.deerclopsattacked
				then
				...
				ent:DoTaskInTime(ICESPAWNTIME + 0.03, function()
					ent.deerclopsattacked = nil
				end)
			end
		end
	end
...
local function SpawnIceFx(inst, target)
	...
	for i = 1, num do
		...
		inst:DoTaskInTime(math.random() * 0.25, DoSpawnIceSpike, x + offset.x, z + offset.z)
	end
				...
				if target ~= nil then
					...
					elseif target.components.freezable ~= nil then
						if target.components.freezable:IsFrozen() then
							isfrozen = true
						elseif
							target.components.freezable:ResolveResistance() - target.components.freezable.coldness <= 2
						then
							shouldfreeze = true
						end
					end
	...
	State({
		...
		timeline = {
			TimeEvent(2 * FRAMES, function(inst)
				SetLightColour(inst, 0.9)
			end),
			TimeEvent(3 * FRAMES, function(inst)
				SetLightColour(inst, 0.87)
			end),
			TimeEvent(4 * FRAMES, function(inst)
				SetLightColour(inst, 0.845)
			end),
			TimeEvent(5 * FRAMES, function(inst)
				SetLightColour(inst, 0.825)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/deerclops/taunt_grrr")
			end),
			TimeEvent(6 * FRAMES, function(inst)
				SetLightColour(inst, 0.81)
			end),
			TimeEvent(7 * FRAMES, function(inst)
				SetLightColour(inst, 0.8)
			end),
	...
	State({
		...
		onupdate = function(inst)
			if inst.sg.statemem.target ~= nil then
				if inst.sg.statemem.target:IsValid() then
					...
					if
						dx * dx + dz * dz < 256
						and math.abs(anglediff(inst.Transform:GetRotation(), math.atan2(-dz, dx) / DEGREES)) < 45
					then
		...
		timeline = {
			TimeEvent(FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/deerclops/attack", nil)
			end),
			TimeEvent(4 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/deerclops/step", nil, 0.7)
			end),
			TimeEvent(6 * FRAMES, function(inst)
				SetLightValue(inst, 0.97)
			end),
			TimeEvent(7 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 1, 0.2)
			end),
			TimeEvent(8 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 0.99, 0.15)
			end),
			TimeEvent(9 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 0.97, 0.05)
			end),
			TimeEvent(10 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 0.96, 0)
			end),
			TimeEvent(11 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 1.01, 0.35)
			end),
			TimeEvent(12 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 1, 0.3)
			end),
			TimeEvent(13 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 0.95, 0.05)
			end),
			TimeEvent(14 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 0.94, 0)
			end),
			TimeEvent(15 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 1, 0.3)
			end),
			TimeEvent(16 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 0.99, 0.25)
			end),
			TimeEvent(17 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 0.92, 0.05)
			end),
			TimeEvent(18 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 0.9, 0)
				inst.sg.statemem.target = nil
				inst.SoundEmitter:PlaySound("dontstarve/creatures/deerclops/taunt_howl", nil, 0.4)
			end),
			TimeEvent(19 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/deerclops/laser")
				SpawnLaser(inst)
				SetLightValueAndOverride(inst, 1.08, 0.7)
			end),
			TimeEvent(20 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 1.12, 1)
			end),
			TimeEvent(21 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 1.1, 0.9)
			end),
			TimeEvent(22 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 1.06, 0.4)
			end),
			TimeEvent(23 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 1.1, 0.6)
			end),
			TimeEvent(24 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 1.06, 0.3)
			end),
			TimeEvent(25 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 1.05, 0.25)
			end),
			TimeEvent(26 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 1.1, 0.5)
			end),
			TimeEvent(27 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 1.08, 0.45)
			end),
			TimeEvent(28 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 1.05, 0.2)
			end),
			TimeEvent(29 * FRAMES, function(inst)
				SetLightValueAndOverride(inst, 1.1, 0.3)
			end),
			TimeEvent(30 * FRAMES, function(inst)
				inst.sg.statemem.lightval = 1.1
			end),
			TimeEvent(32 * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound("dontstarve/creatures/deerclops/taunt_grrr", nil, 0.5)
			...
			TimeEvent(43 * FRAMES, function(inst) end),
...
CommonStates.AddCombatStates(states, {
	hittimeline = {
		TimeEvent(0 * FRAMES, function(inst)
			inst.SoundEmitter:PlaySound("dontstarve/creatures/deerclops/hurt")
		end),
	},
	attacktimeline = {
		TimeEvent(0 * FRAMES, function(inst)
	...
	deathtimeline = {
		...
		TimeEvent(4 * FRAMES, function(inst)
			SetLightValue(inst, 1.025)
		end),
		TimeEvent(5 * FRAMES, function(inst)
			SetLightValue(inst, 1.045)
		end),
		TimeEvent(6 * FRAMES, function(inst)
		...
		TimeEvent(34 * FRAMES, function(inst)
			SetLightValue(inst, 1.01)
			SetLightColour(inst, 0.85)
		end),
		TimeEvent(35 * FRAMES, function(inst)
			SetLightValue(inst, 1)
			SetLightColour(inst, 0.75)
		end),
		TimeEvent(36 * FRAMES, function(inst)
			SetLightColour(inst, 0.7)
		end),
		TimeEvent(48 * FRAMES, function(inst)
			if inst.Light ~= nil then
				local k = 1
...
CommonStates.AddSleepStates(states, {
	starttimeline = {
		TimeEvent(1 * FRAMES, function(inst)
			SetLightValue(inst, 0.995)
		end),
		TimeEvent(2 * FRAMES, function(inst)
			SetLightValue(inst, 0.99)
		end),
		TimeEvent(3 * FRAMES, function(inst)
			SetLightValue(inst, 0.98)
		end),
		...
		TimeEvent(5 * FRAMES, function(inst)
			SetLightValue(inst, 0.96)
		end),
		TimeEvent(6 * FRAMES, function(inst)
			SetLightValue(inst, 0.95)
		end),
		TimeEvent(7 * FRAMES, function(inst)
			SetLightValue(inst, 0.945)
		end),
...
return StateGraph("SGkochosei_deerclops", states, events, "idle", actionhandlers)
