...
--------------------------------------------------------------------------
-- CHOP WORK HELPERS
-- Dùng khi inst.chop_target được set: work cây và quyết định bước tiếp theo.

-- Gọi tại hit frame của mỗi combo swing để damage cây.
local function DoChopHit(inst)
    local target = inst.chop_target
    ...
    if workable ~= nil and workable:CanBeWorked() then
        workable:WorkedBy(inst, 1)
    end
end

-- Gọi tại animqueueover: tiếp tục combo hoặc thoát về finisher/idle.
-- next_chop_state: state combo tiếp theo ("attack_combo2" hoặc "attack_combo1a" hoặc "attack_combo1")
-- Trả về true nếu đã xử lý (state đã chuyển), false nếu không có chop_target.
local function HandleChopQueueOver(inst, next_chop_state)
    local target = inst.chop_target
    ...
    if workable == nil or not workable:CanBeWorked() then
        ...
        inst.sg:GoToState("idle", IDLE_FLAGS.Calm)
        return true
    end
    ...
    -- Cây còn nhiều hit → tiếp tục combo
    inst.sg:GoToState(next_chop_state, target)
    return true
end
...
local actionhandlers = {
	...
	-- CHOP: set chop_target rồi dùng combo attack có sẵn
	ActionHandler(ACTIONS.CHOP, function(inst)
		local ba = inst:GetBufferedAction()
		if ba ~= nil and ba.target ~= nil and ba.target:HasTag("CHOP_workable") then
			inst.chop_target = ba.target
			return "attack_combo1"
		end
	end),
	-- DIG: dùng state chuyên dụng quad → butt → quad
	ActionHandler(ACTIONS.DIG, function(inst)
		return inst:GetBufferedAction().target:HasTag("DIG_workable") and "dig_work"
	end),
...
local states = {
	...
	State({
		...
		timeline = {
			...
			FrameEvent(32, function(inst)
				...
				DoChopHit(inst)  -- CHOP: work cây nếu đang chặt
			end),
			FrameEvent(33, function(inst)
				DoComboArcAttack(inst, inst.sg.statemem.targets)
				DoComboArcWork(inst)
	...
	State({
		...
		onenter = function(inst)
			local ba = inst:GetBufferedAction()
			if ba == nil or ba.target == nil or not ba.target:IsValid() then
				inst.sg:GoToState("idle")
				return
			end
