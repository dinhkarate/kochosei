...
local function onattackedfn(inst, data)
	if
		...
		and not CommonHandlers.HitRecoveryDelay(inst)
		and (not inst.sg:HasStateTag("busy") or inst.sg:HasStateTag("frozen"))
	then
...
local function onattackfn(inst)
	...
	then
		...
		if
			inst.components.combat ~= nil
			and inst.components.combat.target ~= nil
			and inst.components.combat.target:HasTag("beehive")
		then
...
local function DoFootstep(inst) end
...
local states = {
		...
		timeline = {
			...
			TimeEvent(20 * FRAMES, function(inst)
				...
				if not inst.components.timer:TimerExists("GroundPound") then
					inst.components.timer:StartTimer("GroundPound", TUNING.BEARGER_NORMAL_GROUNDPOUND_COOLDOWN - 3)
				end
	...
	State({
		...
		events = {
			EventHandler("animover", function(inst)
				inst.num_good_food_eaten = inst.last_food_good and inst.num_good_food_eaten + inst.last_food_good
					or inst.num_good_food_eaten
	...
	State({
		...
		onenter = function(inst)
			local anim = (inst.components.combat.target and not inst.components.combat.target:HasTag("beehive"))
					and "charge_pre"
				or "walk_pre"
			if GoToStandState(inst, "bi") then
	...
	State({
		...
		onenter = function(inst)
			...
			if inst.components.combat and inst.components.combat.target and math.random() < 0.5 then
...
return StateGraph("kochosei_bearger", states, events, "idle", actionhandlers)
