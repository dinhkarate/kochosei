...
local BossjtConsumeAllMaterials = DefaultValue(TUNING.BOSSJT_CONSUME_ALL_MATERIALS, false)
local BossjtByMenus = DefaultValue(TUNING.BOSSJT_BY_MENUS, false)
local BossjtAcceptsstacks = DefaultValue(TUNING.BOSSJT_ACCEPTSSTACKS, false)
		...
		self.Open = function(self, container, doer, ...)
			...
			if widget.button_info ~= nil then
				...
				for i = 1, #widget.button_info, 1 do
					self.button[i] = self.button:AddChild(
						ImageButton(
							"images/ui.xml",
							"button_small.tex",
					...
					self.button[i].inst:ListenForEvent("continuefrompause", function()
						if TheInput:ControllerAttached() then
							self.button[i]:Hide()
						else
							self.button[i]:Show()
...
local function CreateChest(chest_name, widget, param)
	...
	if param ~= nil then
		for key, value in pairs(param) do
			params[chest_name][key] = value
		end
	end

	for y = 2, 0, -1 do
		for x = 0, 2 do
			table.insert(params[chest_name].widget.slotpos, Vector3(75 * x - 75 * 2 + 75, 75 * y - 75 * 2 + 75, 0))
		end
	end
...
CreateChest("kochosei_altar", {
	...
	itemtestfn = function(inst, item, slot)
		...
		return item:IsValid() and item.components.leader == nil and not (item:HasTag("bundle"))
	end,
})
...
AddModRPCHandler("kochosei_altar", "synthesis", function(player, kochosei_altar)
	if kochosei_altar.components.container ~= nil and kochosei_altar.components.container:IsOpenedBy(player) then
		local container = kochosei_altar.components.container

		if container:IsEmpty() then
			return player.components.talker:Say("Mang đồ nhét vô")
		end
		...
		local find_fn = function(recipe)
			...
			if recipe.character_ingredients and #recipe.character_ingredients > 0 then
				return false
			end
			...
			local amount = 0
			local hasIngredient = Every(recipe.ingredients, function(ingredient, index)
				if BossjtRigorous then
					local item = container:GetItemInSlot(index)
					local item_prefab = (item and item.prefab) or nil
					local item_stacksize = (item and item.components.stackable and item.components.stackable.stacksize)
						or 1
					local ingredient_type = (ingredient and ingredient.type) or nil
					local ingredient_amount = (ingredient and ingredient.amount) or 1
					if item_prefab == ingredient_type and item_stacksize == ingredient_amount then
						return true
					end
					return false
				end
				...
				local result = container:Has(ingredient.type, ingredient.amount) == true
				if result == true then
		...
		if recipe == nil and BossjtAcceptsstacks == true and BossjtByMenus == true then
			recipe = FindItem(AllRecipes, find_fn)
		end
		...
		if recipe == nil then
			...
			return player.components.talker:Say("Hình như sai nguyên liệu rồi!")
		end
		...
		if prefab == nil then
			return player.components.talker:Say("Ừ thì, chắc là thành công rồi đó")
		end
		...
		if prefab.components.inventoryitem == nil then
			-- prefab:Remove()
			return player.components.talker:Say("É é é ")
		end
