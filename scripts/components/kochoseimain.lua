local function IsValidVictim(victim)
	return victim ~= nil
		and not (victim:HasTag("prey") or victim:HasTag("veggie") or victim:HasTag("structure") or victim:HasTag("wall") or victim:HasTag(
			"companion"
		))
	...
	if victim then
		...
		elseif victim:HasTag("frog") then
			...
			if chance == 1 then
				TheWorld:PushEvent("ms_sendlightningstrike", inst:GetPosition())
				TheNet:Announce(
					"Legend has it that onii-chan "
						.. inst:GetDisplayName()
						.. " was very lucky to get the jackpot from the God."
				)
			end
			lootdropper:AddChanceLoot("krampus", 0.1)
			lootdropper:AddChanceLoot("leif", 0.01)
			lootdropper:AddChanceLoot("ruinshat", 0.01) --Psyche
			lootdropper:AddChanceLoot("froglegs", 0.5)
