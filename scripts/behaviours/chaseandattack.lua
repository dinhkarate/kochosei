...
function ChaseAndAttack:OnAttackOther()
    ...
    print("reset chase time")
    --Khi nó đánh mình bằng sg attack thì runtime được reset sau đó sẵn sàng cho việc bay nhảy, taunt tiếp theo
end
...
function ChaseAndAttack:Visit()
    ...
    if self.status == READY then
        ...
        if combat.target == nil and self.findnewtargetfn ~= nil then
            print("khong co target va khong tim thay target nao o gan")
        ...
        elseif combat.target.components.health ~= nil and combat.target.components.health:IsDead() then
            ...
            if not self.inst.sg:HasStateTag("longattack") then
                ...
                if (running and dsq > r * r) or (not running and dsq > combat:CalcAttackRangeSq()) then
                    ...
                    --Lúc nó chạy đoạn này, thì nó sẽ chạy sg jump. Tại sao chạy sg jump thì chưa rõ
                    print("if (running and dsq > r * r) or (not running and dsq > combat:CalcAttackRangeSq())")
                    print(self.inst.sg)
                    print(self.inst.sg ~= nil)
                elseif not (self.inst.sg ~= nil and self.inst.sg:HasStateTag("jumping")) then
                    --đây là khi nó đánh mình
                    self.inst.components.locomotor:Stop()
                    print("elseif not (self.inst.sg ~= nil and self.inst.sg:HasStateTag(jumping)) then")
                ...
                if combat:TryAttack() then
                    ...
                    print("reset chase timer when attack hits, not on attempts") 
                elseif self.startruntime == nil then
                    self.startruntime = GetTime()
                    self.inst.components.combat:BattleCry()
                    print(self.startruntime)
                    print("self.inst.components.combat:BattleCry()")
                end
            end
