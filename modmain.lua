...
local function spawntom(inst, data)
	if data.name == "Spawn_Tom" and inst.components.container then
		local getallitem = inst.components.container:ReferenceAllItems()
...
local NHAC_THA_T_RA = GLOBAL.Action({ distance = 1 })
...
local NHAC_BAT_LAI = GLOBAL.Action({ distance = 1 })
...
AddComponentAction("SCENE", "follower", function(inst, doer, actions, right)
	if right then
		if inst:HasTag("kochosei_enemy") and not inst:HasTag("clone_tha_xich") then
...
AddPrefabPostInit("cay_hoa_sang", function(inst)
    ...
    if TARGET_MUST_TAGS and TARGET_MUST_TAGS_index then
        TARGET_MUST_TAGS = { "_combat", "monster" }
