...
local listmodneedcheck = {
    ...
    "2477561322", "2958351483","2526778484"
}
TUNING.KOCHOSEI_CHECKMOD = nil
TUNING.KOCHOSEI_CHECKWIFI = 0 -- Wifi mà, không phải waifu

for k, v in ipairs(_G.KnownModIndex:GetModsToLoad()) do
...
for _, v in ipairs(listmodneedcheck) do
    for _, listmod in ipairs(_G.KnownModIndex:GetModsToLoad()) do
...
local listiteminv = {
    "miohm", "kocho_lotus_flower", "kocho_lotus", "kocho_lotus_flower_cooked",
    "kochosei_purplemagic", "miku_usagi_backpack", "kocho_purplesword",
    "kocho_miku_cos", "kocho_miku_back", "kochosei_umbrella",
    "kochosei_demonlord", "kochosei_hat1", "kochosei_hat2", "kochosei_hat3",
    "ms_kochosei_hat2", "ms_kochosei_hat3", "kochotambourin",
    "kochosei_lantern", "kochosei_apple", "kochosei_apple_cooked", "kochobook",
    "kochosei_hatfl", "lucky_hammer", "kochosei_ancient_books"
}
...
PrefabFiles = {
    ...
    "kochosei_apple_plantables", "kochosei", "kochosei_none", "miohm",
    "kocho_bearger", "kochotambourin", "kochosei_hat", "kochosei_lantern",
    "kochosei_streetlight", "kochosei_streetlight1_musicbox", "kochosei_enemy",
    "kochosei_enemyb", "lavaarena_blooms_kocho", "kochosei_house",
    "kochosei_purplemagic", "kochosei_magicbubble", "miku_usagi_backpack",
    "kocho_purplesword", "kocho_lotus_flower", "kocho_lotus", "kocho_lotus2",
    "kocho_miku_cos", "kochofood", "kocho_stalk", "kochosei_wishlamp",
    "kochosei_torigate", "kochodragonfly", "kochodeerclops",
    "kochosei_umbrella", ---RAIDENFU---
    "kochosei_demonlord", "lucky_hammer", "kochosei_ancient_books", "bienbao",
    "kochosei_enemy_d"
}
...
local function namngua(inst)
    if inst.prefab ~= "kochosei" then return end

    if inst.sg:HasStateTag("knockout") then
        inst.sg.statemem.cometo = nil
    elseif not (inst.sg:HasStateTag("sleeping") or
        inst.sg:HasStateTag("bedroll") or inst.sg:HasStateTag("tent") or
...
AddPrefabPostInitAny(function(inst)
    ...
    if (inst.components.equippable and inst.components.inventoryitem) or
        inst.components.armor or inst.components.weapon or inst.prefab ==
        "dragon_scales" or inst.prefab == "deerclops_eyeball" or inst.prefab ==
        "bearger_fur" or inst:HasTag("light") and not inst.components.tradeable then
...
local function CustomOnHauntkochosei(inst, haunter)
    if haunter and haunter.prefab == "kochosei" then
        ...
        if inst.components.health then inst.components.health:Kill() end -- Tôi năm nay 2k1 nhưng chưa thấy ai độc ác như này, hồi sinh bằng bứm ạ
...
InsertIngredientValues({"kochosei_apple"}, {fruit = 1, apple = 1})
